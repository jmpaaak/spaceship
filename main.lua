...
function love.load()
    ...
    elseif capturePhase == "full-loop-relaunch" then
        ...
        world.nearbyPlanets = function() return {} end
        -- docs/feedback/INBOX.md 처리대기 항목 15(a): the old beginReturn/
        -- "returning" phase and in-flight slot machine are fully removed --
        -- returning to Earth now settles immediately via the "r" key
        -- (expedition.returnToEarth), with no travel-down/slot-spin loop.
        scene:keypressed("r") -- return to Earth -> settlement
