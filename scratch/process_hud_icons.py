#!/usr/bin/env python3
"""ComfyUI generate + chroma-key post-process for HUD icon group (4) slice 1.
...
ICONS = [
    {
        "path": "assets/effects/hud_coin.png",
        "seed": 20260905301,
        "prompt": (
            "pixel art, 16-bit, tiny solid gold coin HUD icon, simple circular "
            "gold coin with a star stamp, single solid-color currency symbol "
            "centered with padding, isolated game HUD sprite, black background, "
            "no text, no numbers, no letters, no scene, no frame, no glow bloom"
        ),
        "qa_name": "gold coin cash HUD icon",
    },
    {
        "path": "assets/effects/hud_shield.png",
        "seed": 20260905302,
        "prompt": (
            "pixel art, 16-bit, tiny solid cyan-blue kite shield HUD icon, "
            "flat-top pointed-bottom heater shield, single solid-color hull "
            "durability symbol centered with padding, isolated game HUD sprite, "
            "black background, no text, no numbers, no letters, no scene, no frame"
        ),
        "qa_name": "cyan kite shield HUD icon",
    },
    {
        "path": "assets/effects/hud_speed.png",
        "seed": 20260905303,
        "prompt": (
            "pixel art, 16-bit, tiny solid cyan-green speedometer HUD icon, "
            "semicircular analog gauge with a needle, single solid-color speed "
            "symbol centered with padding, isolated game HUD sprite, black "
            "background, no text, no numbers, no letters, no scene, no frame"
        ),
        "qa_name": "cyan-green speedometer HUD icon",
    },
]
...
def update_manifest(path: str, seed: int, qa_name: str, opaque: int, body: tuple[int, int]) -> str:
    ...
    for entry in entries:
        if entry.get("path") == path:
            ...
            entry["height"] = height
            entry["qa"] = (
                f"INBOX group (4) slice 1: {width}x{height} {mode} transparent {qa_name}, "
