#!/usr/bin/env python3
"""ComfyUI generate + chroma-key post-process for HUD icon group (4) slice 2.
...
ICONS = [
    {
        "path": "assets/effects/hud_distance.png",
        "seed": 20260905304,
        "prompt": (
            "pixel art, 16-bit, tiny solid cyan four-pointed nav diamond HUD "
            "icon, simple compass diamond pointing up, single solid-color "
            "distance-from-earth symbol centered with padding, isolated game "
            "HUD sprite, black background, no text, no numbers, no letters, "
            "no scene, no frame, no glow bloom"
        ),
        "qa_name": "cyan nav-diamond distance HUD icon",
    },
    {
        "path": "assets/effects/hud_best.png",
        "seed": 20260905305,
        "prompt": (
            "pixel art, 16-bit, tiny solid gold trophy cup HUD icon, simple "
            "front-view award cup with stem and base, single solid-color "
            "personal-best symbol centered with padding, isolated game HUD "
            "sprite, black background, no text, no numbers, no letters, no "
            "scene, no frame, no glow bloom"
        ),
        "qa_name": "gold trophy personal-best HUD icon",
    },
    {
        "path": "assets/effects/hud_samples.png",
        "seed": 20260905306,
        "prompt": (
            "pixel art, 16-bit, tiny solid amber specimen vial HUD icon, "
            "simple front-view glass flask with cork and glowing sample, "
            "single solid-color samples symbol centered with padding, isolated "
            "game HUD sprite, black background, no text, no numbers, no "
            "letters, no scene, no frame, no glow bloom"
        ),
        "qa_name": "amber specimen-vial samples HUD icon",
    },
]
...
def update_manifest(path: str, seed: int, qa_name: str, opaque: int, body: tuple[int, int]) -> str:
    ...
    for entry in entries:
        if entry.get("path") == path:
            ...
            entry["height"] = height
            entry["qa"] = (
                f"INBOX group (4) slice 2: {width}x{height} {mode} transparent {qa_name}, "
