...
- 캐릭터 identity, 사진 사용 권한, 라이선스, 팔레트, 런타임 렌더링 규칙은 더 엄격한 안전장치로 유지한다. 단, 과거의 provider-only 문구는 승인된 Asset Studio 워크플로를 금지하거나 우회시키는 근거가 될 수 없다. 특히 사용자가 직접 제공하기로 한 함선·지구·행성 등은 해당 identity/출처 규칙 없이는 사진 변환으로 대체하지 않는다.
