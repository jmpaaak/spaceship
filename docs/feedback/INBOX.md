...
(77) **은하 상점 장비 구매·판매·정찰선 문구·회복 밸런스 정리** (msg `1546761251697328169`, R1 완료 후 진행)
  - (1) **완료 (2026-09-08):** 은하계 별 상점에서 한 번에 구매 가능한 장비는 **최대 1개**로 제한한다. 담당: 새 순수 모듈 `game/shop_gear_rules.lua` + `game/scenes/play_shop.lua` 소비. 상점별 성공 구매를 독립 추적하고 첫 구매 후 추가 구매 콜백을 차단하며 버튼을 비활성 문구로 표시한다. 실패한 구매·다른 상점·지구 업그레이드·슬롯 구매는 제한에 포함하지 않는다. `game/tests/shop_gear_rules.lua` 및 `make test` GREEN.
