...
지구에서 출발해 가능한 한 멀리 탐험하며 행성 표본을 수집하고, 운석·우주 쓰레기와의 충돌을 피하며, 귀환 슬롯을 거쳐 표본을 돈으로 바꾸고 우주선을 강화해 다시 도전하는 모바일 로그라이트.

## 플랫폼과 화면

- 핸드폰 **세로 모드 전용** 플레이
- 내부 해상도 `720×1280` (9:16 세로, Balatro 수준 픽셀 밀도; 예전 `180×320`은 너무 낮아 폐기), nearest-neighbor 확대
...
6. 귀환은 연료 고갈이 아니라 명시적 귀환으로 시작한다.
...
- 명시적 귀환(`beginReturn`)으로 returning phase로 전환한다. 연료 고갈은 더 이상 트리거가 아니다.
...
- 최고 높이 영구 저장
- 세로 `720×1280` 실제 LÖVE 캡처와 헤드리스 테스트
