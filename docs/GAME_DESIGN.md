...
지구에서 출발해 가능한 한 멀리 탐험하며 행성 표본을 수집하고, 연료가 다하면 귀환 슬롯을 거쳐 표본을 돈으로 바꾸고 우주선을 강화해 다시 도전하는 모바일 로그라이트.
...
- 조이스틱/좌우 기동의 추가 이동 거리는 자동 상승과 같은 비율로 연료를 추가로 소모한다. 공회전 상승은 기존 연료 소모만 쓴다
- 작은 미니맵에 각 은하계의 중심(홈 은하는 지구)과 내 위치를 표시한다. 우주에 하드 경계는 없고, 차트 반경을 벗어나면 미니맵이 이탈 거리와 지구 방향만 알려 준다

## 원정 루프

1. 지구에서 현재 우주선과 강화를 확인하고 출발한다.
2. 연료를 소모하며 지구에서 멀어진다.
