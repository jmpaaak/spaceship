...
- `returning` 진입 뒤 실제 하강·슬롯 실행·안전 귀환 정산은 아직 미구현이다.
...
- engine-hosted 귀환 테스트를 먼저 추가하고 `returning` 자동 하강, 유한 슬롯 기회 사용, 지구 도착 시 `settlement` 전이를 구현한다.

## 완료 조건

- `make verify LOVE=/Users/jm/.local/bin/love` 통과 (`SPACESHIP_UNIT_OK`, bundle 23 entries)
- 세로 실제 런타임 캡처 `540×960` 확인
- 안전 귀환 정산·상점·재출발과 파괴 실패 자동 테스트는 후속 slice
