...
2026-09-05 — 모바일 UI 전체 점검 (7) 귀환 버튼 확대 + (2)(3) 완료 확인

- `game/scenes/play.lua`:
  - `ascendReturnButton` 200×44px → 300×48px (left=210, right=510, top=1186, bottom=1234).
  - 배경 불투명도 0.7 → 0.85, 텍스트 색상 (0.6,0.85,1,0.9) → (0.75,0.95,1,1).
  - 모서리 반경 6 → 8, 텍스트 y-offset 12 → 14.
- `game/self_test.lua`:
  - 귀환 버튼 80×44px 최소 크기 + 중앙 정렬 검증 어설션 추가.
- INBOX: (2) 미니맵 마커 ×1.5 ✅, (3) 조이스틱 모바일 ✅, (7) 귀환 버튼 ✅ 표시.
- `make verify` GREEN.

## Next Slice

- 모바일 UI 전체 점검: 남은 소항목 확인 (검증 캡처 등)
