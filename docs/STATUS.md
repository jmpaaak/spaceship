...
- 안전 귀환 뒤 표본·슬롯 보상 정산과 상점은 아직 미구현이다.
...
- engine-hosted 정산 테스트를 먼저 추가하고 안전 귀환 시 표본 가치와 슬롯 보상을 돈으로 한 번만 확정하는 정산을 구현한다.

## 완료 조건

- `make verify LOVE=/Users/jm/.local/bin/love` 통과 (`SPACESHIP_UNIT_OK`, `SPACESHIP_SMOKE_OK`, bundle 23 entries)
