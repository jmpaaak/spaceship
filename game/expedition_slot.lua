    ...
    for _, a in ipairs(slotSymbols) do
        for _, b in ipairs(slotSymbols) do
            for _, c in ipairs(slotSymbols) do
                ...
                total = total + probability * M.slotReward({ a, b, c })
...
function M.galaxyDistance(run, galaxyId)
    ...
    if id and not require("game.expedition").homeGalaxies[id] then
...
function M.galaxySlotOddsProfile(galaxyId)
    if not galaxyId or require("game.expedition").homeGalaxies[galaxyId] then
        return "solar"
    end
    local h = 0
    for i = 1, #galaxyId do
...
function M.earthSlotSpin(run, galaxyId, rolls)
    ...
    if matchCount >= 2 and matchSymbol then
        ...
        elseif matchSymbol == "PART" then
            ...
            rewardPart = require("game.expedition").rollGearOffer(run, filteredPool, partRolls)
        end
